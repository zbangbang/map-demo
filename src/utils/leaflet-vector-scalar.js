...
L.ScalarLayer = (L.Layer ? L.Layer : L.Class).extend({
  ...
  getCanvasLayer: function getCanvasLayer(map) {
  ...
  setOpacity: function setOpacity(opacity) {
    ...
    L.DomUtil.setOpacity(this._canvasLayer._canvas, opacity);
    this.options.opacity = opacity;
    console.log('setOpacity', this.options.opacity);
  },
  ...
  _onMouseMove: function (e) {
    ...
    var gridValue = self._scalar ? self._scalar.interpolatePoint(
        pos.lng,
        pos.lat
    ) : null;
    if(self.options.numType == 'temperature') {
      gridValue -= 273.15;
    }
    var htmlOut =gridValue ? gridValue.toFixed(2) : null;

    // 添加跟随鼠标信息显示
    let div = document.getElementsByClassName("scalarNum")[0];
    if(this.options.opacity == 0) {
      div.style.display = 'none';
    } else {
      div.style.left = e.containerPoint.x + 10 + 'px';
      div.style.top = e.containerPoint.y + 10 + 'px';
  ...
  _initScalar: function _initScalar(self) {
    ...
    this._map.on("movestart", self._hideScalar, this);

    this._map.on("moveend", self._clearAndRestart, this);

    this._map.on("zoomstart", self._hideScalar, this);

    this._map.on("zoomend", self._clearAndRestart, this);

    this._map.on("resize", self._clearScalar);

    window.onresize = () => {self._clearAndRestart}
  ...
  _hideScalar: function _hideWind() {
    // console.log('viewset');
    console.log('_hideScalar', this.options.opacity);
  ...
  _clearAndRestart: function _clearAndRestart() {
    console.log('_clearAndRestart', this.options.opacity);
    if(this.options.opacity == 0) return;
    ...
    setTimeout(function () {
      ...
      if (canvas) {
        ...
        canvas.style.opacity = that.options.opacity;
      }
    }, 0);
  },
  ...
  _destroyScalar: function _destroyScalar() {
    ...
    this._map.off("movestart", this._hideScalar, this);
    this._map.off("moveend", this._clearAndRestart, this);
    this._map.off("zoomstart", this._hideScalar, this);
    this._map.off("zoomend", this._clearAndRestart, this);
    this._map.off("resize", this._clearScalar);
...
var Scalar = function Scalar(params) {
  ...
  var OVERLAY_ALPHA = Math.floor(0.8 * 255);
  var TRANSPARENT_BLACK = [0, 0, 0, 0]; //default canvas's grid data color

  var MIN_VELOCITY_INTENSITY // velocity at which particle intensity is minimum (m/s)
  if(params.minValue) {
    MIN_VELOCITY_INTENSITY = params.minValue;
  } else if(params.minValue == 0) {
    MIN_VELOCITY_INTENSITY = 0;
  } else {
    MIN_VELOCITY_INTENSITY = 193;
  }
  ...
  var createScalarBuilder = function createScalarBuilder(res) {
    ...
    if (res.length > 1){
      ...
      _data = _data1.data.map(function (v, i) {
        // 向量中有 负值，需取绝对值
        return Math.hypot(v, _data2.data[i]);
      ...
      for (var y = bounds.y; y <= bounds.yMax; y += 2) {
        ...
        if (coord) {
          ...
          if (isFinite(λ)) {
            ...
            if (isValue(scalar)) {
              // 陆地数据为NaN时，陆地部分绘制透明
              color = isNaN(scalar) ? SCALE.gradient(scalar, 0) : SCALE.gradient(scalar, OVERLAY_ALPHA);
            }
          }
        }
